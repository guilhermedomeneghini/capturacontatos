...
const friendlyBlockedMessage = "Nao foi possivel concluir a coleta no momento. O Google Maps pode ter alterado sua estrutura ou bloqueado temporariamente a automacao.";
...
async function acceptConsentIfVisible(page) {
    ...
    for (const label of labels) {
        ...
        if ((await button.count()) > 0) {
            ...
            await (0, delay_1.randomDelay)(700, 1200);
            return;
        }
    }
}
async function collectResultLinks(page, limit, onProgress) {
    const links = new Set();
...
async function extractLeadFromPlace(page) {
...
async function scrapeGoogleMaps(query, limit, onProgress) {
    let browser;
    try {
        ...
        await detectCaptchaOrBlock(page);
        await (0, delay_1.randomDelay)(1300, 2200);
